...
<?php
if(empty($_POST["uname"])||empty($_POST["pwd"])||empty($_POST["email"]))
...
$number=$row[0];
$rs=mysql_query("select name from user");
for($i=1;$i<=$number;$i++)
{
	$row=mysql_fetch_array($rs);
	if(strcmp($_POST["uname"],$row['name'])==0)
	break;
}
if($i!=($number+1))
	{echo "Username already already exists Please Try Another One";
...
{
	mysql_query("insert into user values('".$_POST["uname"]."','','".$_POST["email"]."','".$_POST["pwd"]."','','','".$_POST["colour"]."')");
	mysql_close();
